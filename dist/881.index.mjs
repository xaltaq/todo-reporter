...
var text_table = __webpack_require__(418);
// EXTERNAL MODULE: external "os"
var external_os_ = __webpack_require__(2037);
// EXTERNAL MODULE: ./node_modules/eol/eol.js
var eol = __webpack_require__(5288);
;// CONCATENATED MODULE: ./node_modules/leasot/dist/lib/reporters/table.js
...
function outputFooter(todos) {
    ...
    if (!total) {
        return external_os_.EOL + ' ' + log_symbols.success + ' No todos/fixmes found';
    }
    let msg = total + ' todo' + (total === 1 ? '' : 's');
    msg += '/fixme' + (total === 1 ? '' : 's') + ' found';
    return external_os_.EOL + ' ' + (total ? log_symbols.error : log_symbols.success) + ' ' + msg;
}
function outputTable(todos) {
    ...
    const mapTodo = (item, index) => {
        let text = source.cyan(item.text);
        if (item.ref) {
            text = source.gray('@' + item.ref) + ' ' + text;
        }
        const line = ['', source.gray('line ' + item.line), source.green(item.tag), text];
    ...
    let t = text_table(todos.map(mapTodo), {
        stringLength(str) {
    ...
    t = eol.split(t)
        .map(function (el, i) {
        return headers[i] ? external_os_.EOL + source.underline(headers[i]) + external_os_.EOL + el : el;
    })
        .join(external_os_.EOL);
    contents += t + external_os_.EOL;
    return contents;
}
/**
...
//# sourceMappingURL=881.index.mjs.map